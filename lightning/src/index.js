...
const uri =
  "mongodb+srv://" +
  process.env.REACT_APP_MONGO_DB_ID +
  ":" +
  process.env.REACT_APP_MONGO_DB_SECRET +
  "@jobs.tcmdh.mongodb.net/jobs?retryWrites=true&w=majority";
const client = new MongoClient(uri);

// pretend reading from file system is like a database lol
const jobDb = JSON.parse(
  fs.readFileSync("../droplets/input/json/jobData.json", "utf-8")
);
const keywords = JSON.parse(
  fs.readFileSync("../droplets/output/json/languagesData.json", "utf-8")
);
...
app.get("/api/companies", (req, res) => {
  ...
  jobDb.forEach((obj) => {
    ...
    if (obj.title.toLowerCase().indexOf(req.query.q.trim().toLowerCase()) < 0)
      return;
...
app.get("/api/majors", (req, res) => {
  ...
  let foundJobs = jobDb.filter(
    (job) =>
      job.title.toLowerCase().indexOf(req.query.q.trim().toLowerCase()) >= 0
  ); // fix this
  ...
  let withPercentages = top10.map((el) => {
    el.percent = (el.count / currAmt).toFixed(5);
    return el;
  });
...
app.get("/api/types", (req, res) => {
  ...
  Object.keys(typesObj).forEach((l) => {
    arrayTypes[index] = {
      type: l.charAt(0).toUpperCase() + l.substr(1),
      count: typesObj[l],
    };
    index++;
  });
