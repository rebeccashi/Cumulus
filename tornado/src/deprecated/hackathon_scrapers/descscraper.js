      ...
      fetch("https://www.indeed.com/viewjob?jk=" + fileArr[i], {
        ...
        .then((data) => {
          ...
          if (data.indexOf("Captcha") >= 0) {
            fs.writeFileSync(
              "../data/output/json/descriptions4.json",
              JSON.stringify(desc)
            );
          ...
          let salarycatcher =
            /window\._initialData=JSON\.parse\('\{(.*)\}'\);<\/script>/g;
          ...
          desc.push({
            id: dataobj.jobKey,
            title:
              dataobj.jobInfoWrapperModel?.jobInfoModel?.jobInfoHeaderModel
                ?.jobTitle,
            company:
              dataobj.jobInfoWrapperModel?.jobInfoModel?.jobInfoHeaderModel
                ?.companyName,
            location:
              dataobj.jobInfoWrapperModel?.jobInfoModel?.jobInfoHeaderModel
                ?.formattedLocation,
            qualifications:
              dataobj.jobDescriptionSectionModel?.qualificationsSectionModel
                ?.content ||
              dataobj.jobInfoWrapperModel?.jobInfoModel
                ?.jobDescriptionSectionModel?.qualificationsSectionModel
                ?.content,
            type: dataobj.jobInfoWrapperModel?.jobInfoModel
              ?.screenerRequirementsModel?.jobType
              ? dataobj.jobInfoWrapperModel?.jobInfoModel
                  ?.screenerRequirementsModel?.jobType["Job Type:"]
              : null,
            salary: dataobj.jobInfoWrapperModel?.jobInfoModel
              ?.screenerRequirementsModel?.salaryInfoModel
              ? dataobj.jobInfoWrapperModel?.jobInfoModel
                  ?.screenerRequirementsModel?.salaryInfoModel["Salary:"]
                  ?.salaryText
              : null,
            description:
              dataobj.jobInfoWrapperModel?.jobInfoModel?.sanitizedJobDescription
                .content,
          });
