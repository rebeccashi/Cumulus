...
files.forEach((file, index) => {
  process.stdout.write(`Transforming skill ${index-shift}/${totalLength-shift}\r`)
  ...
  if (Object.keys(data.timeseries).length == 0) {
    ...
    // has data object
    ["Nov", "Dec", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct"].forEach((month, index) => {
      const monthObject = {
      ...
      if (index == 11) {
        monthObject.titles = Object.keys(data.titles).map(key => {
          return {
            name: key,
        ...
        monthObject.companies = Object.keys(data.companies).map(key => {
          return {
            name: key,
    ...
    fs.writeFileSync(`./db/skills/${uuid(50)}`, JSON.stringify(newData), { flag: 'wx' })
  }
});